<?php
session_start();
  if(isset($_POST['choice_date_pick'])){
    $_SESSION['pickup'] = $_POST['choice_date_pick'];
  }
  if(isset($_POST['choice_date_pick'])){
    $_SESSION['drop'] = $_POST['choice_date_drop'];
  }
  if(isset($_POST['location'])){
    $location = trim($_POST['location']);
    if(isset($location)){
      if($location === "bengaluru"){
        header("location: bengaluru_bike.php");
        exit();
      }
      if($location === "chennai"){
        header("location: chennai_bike.php");
        exit();
      }
      if($location === "hyderabad"){
        header("location: hyderabad_bike.php");
        exit();
      }
      if($location === "vijayawada"){
        header("location: vijayawada_bike.php");
        exit();
      }
      if($location === "vishakapatnam"){
        header("location: vishakapatnam_bike.php");
        exit();
      }
    }
  }
...
<style type="text/css">
	*
	...
	body
	{
		background-image: linear-gradient(rgba(0,0,0,0.3),rgba(0,0,0,0.3)), url('bike_show_case/bike1.jpg');
...
<body>
	...
	<div style="background-color: rgba(255, 255, 255, 0); border-radius: 0;border-top-right-radius: 5px;border-top-left-radius: 5px; ">
        <h1>Bike Rentals</h1><br>
  	</div>
    <br>
	<form class="form_des" method="post" name="bike_info" action="bike_form.php">
    <div class="location">  
